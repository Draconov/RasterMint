...
def test_drop_layer_into_group_header_keeps_the_layer_inside_that_group():
    stack = [new_effect("Grayscale"), new_effect("Sharpen"), new_effect("Invert"), new_effect("Gaussian Blur")]
    stack[0]["group_id"] = "g1"
    stack[1]["group_id"] = "g1"
    stack[3]["group_id"] = "g1"
    groups = [_group("g1", "Group 1")]

    updated_stack, updated_groups = drop_layer(
        stack,
        groups,
        2,
        0,
    ...
    assert [step["kind"] for step in updated_stack] == ["Grayscale", "Sharpen", "Invert", "Gaussian Blur"]
    assert [step.get("group_id", "") for step in updated_stack] == ["g1", "g1", "g1", "g1"]
    assert [group["id"] for group in updated_groups] == ["g1"]


def test_followup_ui_hooks_exist_for_group_workflow_improvements():
    source = (Path(__file__).resolve().parents[1] / "src/rastermint/qml/pages/LayersPage.qml").read_text(encoding="utf-8")
    backend_source = (Path(__file__).resolve().parents[1] / "src/rastermint/qmlui/backend.py").read_text(encoding="utf-8")

    assert "Qt.ShiftModifier" in source
    assert "Qt.Key_F2" in source
    assert 'qsTr("Duplicate Group")' in source
    assert 'qsTr("Group note")' in source
    assert 'qsTr("Selected group")' in source
    assert 'backend.setLayerGroupOpacity' in source
    assert 'backend.setLayerGroupBlendMode' in source
    assert 'backend.toggleSoloLayerGroup(String(groupHeader.modelData.id))' in source
    assert 'visible: groupEnabledToggle.hovered && !groupIsDragging' in source
    assert 'visible: layerEnabledToggle.hovered && !cardDrag.active' in source
    assert 'backend.dropLayer(source, target, mode, targetGroupId)' in source
    assert 'layerList.contentItem' in source
    assert 'value: Number(root.selectedGroupData.opacity || 1)' in source
    assert 'currentIndex: Math.max(0, backend.layerBlendModes.indexOf(String(root.selectedGroupData.blend_mode || "Normal")))' in source
    assert 'toggleSoloLayerGroup' in backend_source
    assert 'def selectLayerRange(self, index: int)' in backend_source
    assert 'def duplicateLayerGroup(self, group_id: str)' in backend_source
    assert 'def ungroupLayerGroup(self, group_id: str)' in backend_source
