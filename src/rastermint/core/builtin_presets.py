...
BUILTIN_PRESETS: tuple[BuiltinPreset, ...] = (
    ...
    BuiltinPreset("print-clean-cmyk", "Clean CMYK Print", "Clean four-screen AM halftone with conventional CMYK angles."),
    BuiltinPreset("print-vintage-screen", "Vintage Screen Print", "Rough CMYK screens with light registration drift and paper interaction."),
    BuiltinPreset("print-2color-poster", "2-Color Poster", "Bold two-ink spot-colour poster screening."),
    BuiltinPreset("print-3color-riso", "3-Color Risograph", "Three-colour spot print with translucent overprint and mild misregistration."),
    BuiltinPreset("print-newspaper-cmyk", "Newspaper CMYK", "Fine CMYK newspaper screen with dot gain and cheap-paper grain."),
    BuiltinPreset("print-misregistered", "Misregistered Print", "Deliberately offset CMYK separations for a strong vintage registration error."),
    BuiltinPreset("print-cheap-tshirt", "Cheap T-Shirt Print", "Coarse screen, weak ink patches and uneven manual coverage."),
    BuiltinPreset("print-heavy-dot-gain", "Heavy Dot Gain", "Dense CMYK screening with aggressively expanded dots."),
...
PRESET_CONFIGS: dict[str, dict[str, Any]] = {
    "print-clean-cmyk": {"dither": {"algorithm": "Nearest Palette", "mix": 0.0}, "effects": [_effect("Print Lab", mode="CMYK", cell_size=8, dot_shape="Round", dot_gain=4.0, black_mix=90.0, paper_color="#F7F5EF")]},
    "print-vintage-screen": {"dither": {"algorithm": "Nearest Palette", "mix": 0.0}, "effects": [_effect("Print Lab", mode="CMYK", cell_size=10, dot_shape="Round", dot_gain=12.0, black_mix=82.0, registration_error=1.5, roughness=0.24, ink_spread=0.18, paper_grain=0.18, paper_color="#E8DFC9")]},
    "print-2color-poster": {"dither": {"algorithm": "Nearest Palette", "mix": 0.0}, "effects": [_effect("Print Lab", mode="Spot Colors", ink_count=2, cell_size=11, dot_shape="Round", paper_color="#F4EBD7", ink1_color="#E63946", ink2_color="#1D3557", ink1_angle=22.5, ink2_angle=67.5)]},
    "print-3color-riso": {"dither": {"algorithm": "Nearest Palette", "mix": 0.0}, "effects": [_effect("Print Lab", mode="Spot Colors", ink_count=3, cell_size=9, dot_shape="Round", registration_error=1.0, roughness=0.14, paper_grain=0.12, paper_color="#F6F0DE", ink1_color="#FF665E", ink2_color="#0078BF", ink3_color="#FFD447", ink1_opacity=0.86, ink2_opacity=0.86, ink3_opacity=0.82)]},
    "print-newspaper-cmyk": {"dither": {"algorithm": "Nearest Palette", "mix": 0.0}, "effects": [_effect("Print Lab", mode="CMYK", cell_size=5, dot_shape="Round", dot_gain=15.0, black_mix=100.0, paper_grain=0.22, paper_color="#E7E3D7")]},
...
def _clean_preset_base(base: ProcessingSettings | None) -> ProcessingSettings:
    ...
    settings.crop_x = float(base.crop_x)
    settings.crop_y = float(base.crop_y)
    settings.crop_width = float(base.crop_width)
    settings.crop_height = float(base.crop_height)
